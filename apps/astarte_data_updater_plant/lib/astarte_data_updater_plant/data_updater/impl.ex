...
defmodule Astarte.DataUpdaterPlant.DataUpdater.Impl do
  ...
  def handle_internal(state, path, payload, message_id, timestamp) do
    ...
    Core.TriggerExecution.execute_device_error_triggers(
      new_state,
  ...
  def handle_data(state, interface, path, payload, message_id, timestamp) do
    ...
    with :ok <- validate_interface(interface),
      ...
      Core.TriggerExecution.execute_incoming_data_triggers(
        new_state,
        device_id_string,
      ...
      {has_change_triggers, change_triggers} =
        Core.TriggerExecution.value_change_triggers_info(
          new_state,
          interface_id,
          endpoint_id,
          path,
      ...
      if has_change_triggers == :ok do
        :ok =
          Core.TriggerExecution.execute_pre_change_triggers(
            change_triggers,
            new_state.realm,
            device_id_string,
            interface_descriptor.name,
